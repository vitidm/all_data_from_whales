...
def get_swaps_v3_in_last_minutes(minutes):
    now_timestamp = int(time.time())
    past_timestamp = now_timestamp - 60 * minutes

    query = f"""
    {{
      swaps(first: 1000, where: {{timestamp_lte: "{now_timestamp}", origin: "{UNISWAP_V3_ROUTER_ADDRESS}", recipient_not_contains: "{UNISWAP_V3_ROUTER_ADDRESS}"}}, orderBy: timestamp, orderDirection: desc) {{
        id
        timestamp
        origin
        amount0
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Error fetching swaps v3 data: {response.text}")

def get_sushiswap_swaps_in_last_minutes(minutes):
    now_timestamp = int(time.time())
    past_timestamp = now_timestamp - 60 * minutes
    
    query = f"""
    {{
      swaps(first: 200, where: {{timestamp_lte: "{now_timestamp}"}}, orderBy: timestamp, orderDirection: desc) {{
        id
        timestamp
        sender
        amount0In
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Error fetching SushiSwap swaps data: {response.text}")
...
def get_tx_hash_info_sushi(tx_hash):
    # Define la consulta para obtener el par de tokens
    query = f"""
    {{
    transactions(where: {{id: "{tx_hash}"}}) {{
...
while True:
    ...
    swaps_v2 = get_swaps_in_last_minutes(minutes)
    swaps_v3 = get_swaps_v3_in_last_minutes(minutes)
    sushiswap_swaps = get_sushiswap_swaps_in_last_minutes(minutes)
    swaps = swaps_v2 + swaps_v3
    print("-------------- UNISWAP --------------")
    for swap in swaps:
        swap_id = swap['id']
        ...
        if swap_id not in processed_swaps:  # Verificar si el swap ya ha sido procesado
            ...
            try:
                ...
                continue
    print("################# SUSHISWAP MARK ###################")
    for swap in sushiswap_swaps:
        swap_id = swap['id']

        if swap_id not in processed_swaps:
            processed_swaps.add(swap_id)
            swap_info = parse_swap_id(swap['id'])
            # Para los swaps de SushiSwap, la dirección del par ya está en el campo 'id'
            
            try:
                token_symbol, token_name, token_address = get_tx_hash_info_sushi(swap_info['pair_address'])
                if token_symbol != "WETH" and token_symbol != "USDT" and token_symbol != "UNI-V2" and token_symbol != "GETH" and token_symbol != "USDC":
                    tx_hash = swap_info['pair_address']
                    timestamp = datetime.datetime.fromtimestamp(int(swap['timestamp']))
                    timestamp = timestamp.strftime('%Y-%m-%d %H:%M:%S')
